...
u_laminar= zeros(size(y));
for i=1:m
    for j=1:n
        for k=1:p
            u_laminar(i,j,k) = -1/(params.rho_knot*params.molecular_viscosity)*dpdx_laminar*(0.5*y(i,j,k)^2-H*y(i,j,k)); 
	end
    end
end

max_u_laminar = max(max(max(u_laminar)))
a = -10/100*max_u_laminar;
b = 10/100*max_u_laminar;

uvw_pcui(:,:,:,1) = u_laminar +(b-a).*rand(size(y))+a;
