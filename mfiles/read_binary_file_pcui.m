function [x,y,z] =  read_binary_file_pcui(folder, filenameD, istep, params, isvector, isall,numGhost)
%
...
nni = ni/px+2*numGhost;
nnj = nj/py+2*numGhost;
nnk = nk/pz+2*numGhost;
...
for indxz  = 1: pz
    for indxy = 1: py
        for indxx = 1: px               
            coord  = [ffD '.' num2str([100+(indxx-1)*pz*py + (indxy-1)*pz + indxz-1])];
            fid = fopen(coord, 'r');
