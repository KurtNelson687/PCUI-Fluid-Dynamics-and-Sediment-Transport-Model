...
CCCCCCCCCCCCCCCCCCC Added communicator for processing CCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	call MPI_Comm_split(comm3d, npy, myid, hor_comm, ierr )
	call MPI_COMM_RANK( hor_comm, hor_id, ierr )
	call MPI_COMM_SIZE( hor_comm, hor_nprocs, ierr )

! This creates communicator so that one processor from each row can output vertical profiles
	call MPI_Comm_split(comm3d, hor_id, myid, vert_comm, ierr )
	call MPI_COMM_RANK( vert_comm, vert_id, ierr )
