...
% CHARLIE: Maybe modify
% working_folder = '/home/barthur/pcui-3d/examples/pcui_charlie';
working_folder = '../';
...
% CHARLIE: MODIFY, make sure Y is vertical
load 'grid_charlie_512x64.mat';
...
% CHARLIE: rho here is phi in the fortran code
drho = 0.01;
h1 = -0.1;
a = 0.05;
Lw = 0.4;
...
rho_full_pcui(x_pcui > 0.85*params.bx) = 1 + drho/2 + drho/2; %%%%%%%%%%FOR CHARLIE
...
% CHARLIE: phi here is phi2 in the fortran code
phi_pcui = zeros(size(x_pcui));
phi_pcui(x_pcui > 0.85*params.bx)=1; 
write_binary_file_pcui(working_folder, fname_phi_to_PCUI, params, phi_pcui);

% -------------------------------------------------------------------------
% Verify initialized solitary wave
% CHARLIE: Make sure this matches what was output above
% -------------------------------------------------------------------------
% Plot density field
...
rho_full_plot(x_plot > 0.85*params.bx) = 1 + drho/2 + drho/2; %%%%%%%%%%FOR CHARLIE
...
phi_plot = zeros(size(x_plot));
phi_plot(x_plot > 0.85*params.bx)=1; 
