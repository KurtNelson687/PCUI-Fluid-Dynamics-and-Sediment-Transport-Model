cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	double precision cf(0:nni+1,0:nnj+1,0:nnk+1,1:3), 
     &                   phif(0:nni+1,0:nnj+1,0:nnk+1,1:3)
...
C       eps is not used so it is commented out obf 1/12/99
C	eps = TINY(1.D0)
...
c	do j = 1, nnj
c	do i = 1, nni
c	   hbs(i,j,k) = hbs(i,j,k) - rr(i,j,k,5)
c	enddo
c	enddo
c	enddo
...
c	call trid( ax, bx, cx, fx, nnj, 1, nni, n_west, n_east )
...
c	if ( periodic .eq. 1 ) then
c	call trip( az, bz, cz, fz, nni, 1, nnk, n_back, n_frnt )
c	else
