cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine grid

	include "size.inc"
	include "mpif.h"
	include "mpi.inc"
	include "metric.inc"
	include "mygrid.inc"

	integer i, j, k

c.......Set up the horizontal plane
	bx = 0.512D0
c.......Always set "by" as 1.D0 since its value will be assigned in
c.......the function "ReturnDepth"
        by = 1.D0
	bz = 0.256D0

c.......Streeching for each axis
	stretchx = 0
	stretchy = 1
	stretchz = 0

	

	do k = -1, nnk+2
	do j = -1, nnj+2
	do i = -1, nni+2
	   jac(i,j,k) = 0.D0
	   xix(i,j,k) = 0.D0
	   xiy(i,j,k) = 0.D0
	   xiz(i,j,k) = 0.D0
	   etx(i,j,k) = 0.D0
	   ety(i,j,k) = 0.D0
	   etz(i,j,k) = 0.D0
	   ztx(i,j,k) = 0.D0
	   zty(i,j,k) = 0.D0
	   ztz(i,j,k) = 0.D0

	enddo
	enddo
	enddo

c	do k = -1, nnk+2
c	do j = -1, nnj+2
c	do i = -1, nni+2
c	   jac_old(i,j,k) = 0.D0
c	   xix_old(i,j,k) = 0.D0
c	   xiy_old(i,j,k) = 0.D0
c	   xiz_old(i,j,k) = 0.D0
c	   etx_old(i,j,k) = 0.D0
c	   ety_old(i,j,k) = 0.D0
c	   etz_old(i,j,k) = 0.D0
c	   ztx_old(i,j,k) = 0.D0
c	   zty_old(i,j,k) = 0.D0
c	   ztz_old(i,j,k) = 0.D0
c	   xp_old(i,j,k,1) = 0.D0
c	   xp_old(i,j,k,2) = 0.D0
c	   xp_old(i,j,k,3) = 0.D0
c	enddo
c	enddo
c	enddo
        
        
	call metric(1, nni, nnj, nnk,
     <          g11, g12, g13, g21, g22, g23, g31, g32, g33, gcc)

	call qf2c(nni, nnj, nnk, nni1, nnj1, nnk1, jac, jaf,
     <          g11, g12, g13, g21, g22, g23, g31, g32, g33, gcc,
     <          f11, f12, f13, f21, f22, f23, f31, f32, f33, fcc)

	call qf2c(nni1, nnj1, nnk1, nni2, nnj2, nnk2, jaf, jah,
     <          f11, f12, f13, f21, f22, f23, f31, f32, f33, fcc,
     <          h11, h12, h13, h21, h22, h23, h31, h32, h33, hcc)

	call qf2c(nni2, nnj2, nnk2, nni3, nnj3, nnk3, jah, jap,
     <          h11, h12, h13, h21, h22, h23, h31, h32, h33, hcc,
     <          p11, p12, p13, p21, p22, p23, p31, p32, p33, pcc)

	call qf2c(nni3, nnj3, nnk3, nni4, nnj4, nnk4, jap, jar,
     <          p11, p12, p13, p21, p22, p23, p31, p32, p33, pcc,
     <          r11, r12, r13, r21, r22, r23, r31, r32, r33, rcc)

	call qf2c(nni4, nnj4, nnk4, nni5, nnj5, nnk5, jar, jae,
     <          r11, r12, r13, r21, r22, r23, r31, r32, r33, rcc,
     <          e11, e12, e13, e21, e22, e23, e31, e32, e33, ecc)


	return
	end


cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine metric(level, ii, jj, kk, 
     <          q11, q12, q13, q21, q22, q23, q31, q32, q33, qcc)

	include "size.inc"
	include "mpif.h"
	include "mpi.inc"
	include "metric.inc"
	include "mygrid.inc"
	include "para.inc"

	integer level, ii, jj, kk, ii0, jj0, kk0

	double precision q11(-1:ii+2,-1:jj+2,-1:kk+2),
     &                   q12(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q13(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q21(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q22(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q23(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q31(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q32(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   q33(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   qcc(-1:ii+2,-1:jj+2,-1:kk+2) 
	double precision x(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   y(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   z(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   xi(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   et(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   zt(-1:ii+2,-1:jj+2,-1:kk+2)
     
        double precision jab
	double precision xxi, yxi, zxi, xet, yet, zet, xzt, yzt, zzt
	double precision xsx, xsy, xsz, esx, esy, esz, zsx, zsy, zsz

	integer i, j, k, ic
	double precision faci, facj, fack
        character*4 ID

	ii0 = npx * ii
	jj0 = npy * jj
	kk0 = npz * kk

	faci = 1.D0 / dble(ii*px)
        facj = 1.D0 / dble(jj*py)
	fack = 1.D0 / dble(kk*pz)
	

	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   q11(i,j,k) = 0.D0
	   q12(i,j,k) = 0.D0
	   q13(i,j,k) = 0.D0
	   q21(i,j,k) = 0.D0
	   q22(i,j,k) = 0.D0
	   q23(i,j,k) = 0.D0
	   q31(i,j,k) = 0.D0
	   q32(i,j,k) = 0.D0
	   q33(i,j,k) = 0.D0
	   qcc(i,j,k) = 0.D0

        enddo
	enddo
	enddo
c..........Only changed for channel flow. Need to change "0" to "1" for regular cases. YJ 12/3/2013
	if (newrun .eq. 1) then
	  do k = -1, kk+2
	  do j = -1, jj+2
	  do i = -1, ii+2
	     xi(i,j,k) = ( dble(ii0 + i) - 0.5D0 ) * faci
	     zt(i,j,k) = ( dble(kk0 + k) - 0.5D0 ) * fack
	     x(i,j,k) = 0.D0
	     z(i,j,k) = 0.D0
	  enddo
	  enddo
	  enddo
        
	  call coordxz(ii, jj, kk, x, z, xi, zt)
	 
	 
	  call returnDepth(ii, jj, kk, x, z, depth, depth0)

          do k = -1, kk+2
          do j = -1, jj+2
	  do i = -1, ii+2
	     et(i,j,k) = -depth(i, k)+( dble(jj0 + j) - 0.5D0 ) 
     <                  * facj * depth(i, k)
	     y(i,j,k) = 0.D0
	  enddo
          enddo
	  enddo
  
          call coordy(ii, jj, kk, x, y, z, xi, et, zt)

	  if ( level .eq. 1 ) then
	     do k = -1, kk+2
	     do j = -1, jj+2
	     do i = -1, ii+2
	        xp(i,j,k,1) = x(i,j,k)
	        xp(i,j,k,2) = y(i,j,k)
	        xp(i,j,k,3) = z(i,j,k)
	     enddo
	     enddo
	     enddo
	  endif

c	  call bedGeometry
	
c	write(ID, fmt = '(I3)') 100+myid
	
c	  if ( level .eq. 1 .and. newrun .eq. 1 ) then

c	     write(70+myid) xp
c	   close(unit = myid)
           
c	  endif
        else
	   do k = -1, kk+2
	   do j = -1, jj+2
	   do i = -1, ii+2
	      x(i,j,k) = xp(i,j,k,1)
	      y(i,j,k) = xp(i,j,k,2)
	      z(i,j,k) = xp(i,j,k,3)
	   enddo
	   enddo
	   enddo
	    
           do k = -1, kk+2
	   do i = -1, ii+2
	      depth0(i,k) = 0.20
	   enddo
	   enddo


c	   call bedGeometry
	endif
C...... I-face
	
	do k =  0, kk+1
	do j =  0, jj+1
	do i = -1, ii+1
	   xxi = ( x(i+1,j,k) - x(i,j,k) )
	   yxi = ( y(i+1,j,k) - y(i,j,k) )
	   zxi = ( z(i+1,j,k) - z(i,j,k) )
	   xet = 0.25D0 * ( x(i,  j+1,k) - x(i,  j-1,k) 
     <                    + x(i+1,j+1,k) - x(i+1,j-1,k) )
	   yet = 0.25D0 * ( y(i,  j+1,k) - y(i,  j-1,k)
     <                    + y(i+1,j+1,k) - y(i+1,j-1,k) )
	   zet = 0.25D0 * ( z(i,  j+1,k) - z(i,  j-1,k)
     <                    + z(i+1,j+1,k) - z(i+1,j-1,k) )
	   xzt = 0.25D0 * ( x(i,  j,k+1) - x(i,  j,k-1)
     <                    + x(i+1,j,k+1) - x(i+1,j,k-1) )
	   yzt = 0.25D0 * ( y(i,  j,k+1) - y(i,  j,k-1)
     <                    + y(i+1,j,k+1) - y(i+1,j,k-1) )
	   zzt = 0.25D0 * ( z(i,  j,k+1) - z(i,  j,k-1)
     <                    + z(i+1,j,k+1) - z(i+1,j,k-1) )
	   jab = xxi * yet * zzt
     <         + xet * yzt * zxi
     <         + xzt * yxi * zet
     <         - xzt * yet * zxi
     <         - xet * yxi * zzt
     <         - xxi * yzt * zet
	   jab = 1.D0 / jab
	   xsx = ( yet * zzt - yzt * zet )
	   esx = ( yzt * zxi - yxi * zzt )
	   zsx = ( yxi * zet - yet * zxi )
	   xsy = ( zet * xzt - zzt * xet )
	   esy = ( zzt * xxi - zxi * xzt )
	   zsy = ( zxi * xet - zet * xxi )
	   xsz = ( xet * yzt - xzt * yet )
	   esz = ( xzt * yxi - xxi * yzt )
	   zsz = ( xxi * yet - xet * yxi )
	   q11(i,j,k) = jab * ( xsx * xsx + xsy * xsy + xsz * xsz )
	   q12(i,j,k) = jab * ( xsx * esx + xsy * esy + xsz * esz )
	   q13(i,j,k) = jab * ( xsx * zsx + xsy * zsy + xsz * zsz )
cccccccc Note: actually, xix should be xsx/jacobian!! Since it always comes with
cccccccc a multiplication of jab, it does not multiply jab here, just to give
cccccccc more convenience for the future use. However, in some cases if only
cccccccc xix is needed, one needs to be very careful that the real xix
cccccccc is xsx*jab here (jab = 1/jacobian).       comment by YJChou 
	   if ( level .eq. 1 ) then
	      xix(i,j,k) = xsx
	      xiy(i,j,k) = xsy
	      xiz(i,j,k) = xsz
	   endif
	enddo
	enddo
	enddo
	
C...... J-face
	
	do k =  0, kk+1
	do j = -1, jj+1
	do i =  0, ii+1
	   xet = ( x(i,j+1,k) - x(i,j,k) )
	   yet = ( y(i,j+1,k) - y(i,j,k) )
	   zet = ( z(i,j+1,k) - z(i,j,k) )
	   xzt = 0.25D0 * ( x(i,j,  k+1) - x(i,j,  k-1)
     <                    + x(i,j+1,k+1) - x(i,j+1,k-1) )
	   yzt = 0.25D0 * ( y(i,j,  k+1) - y(i,j,  k-1)
     <                    + y(i,j+1,k+1) - y(i,j+1,k-1) )
	   zzt = 0.25D0 * ( z(i,j,  k+1) - z(i,j,  k-1)
     <                    + z(i,j+1,k+1) - z(i,j+1,k-1) )
	   xxi = 0.25D0 * ( x(i+1,j  ,k) - x(i-1,j  ,k) 
     <                    + x(i+1,j+1,k) - x(i-1,j+1,k) )
	   yxi = 0.25D0 * ( y(i+1,j  ,k) - y(i-1,j  ,k) 
     <                    + y(i+1,j+1,k) - y(i-1,j+1,k) )
	   zxi = 0.25D0 * ( z(i+1,j  ,k) - z(i-1,j  ,k) 
     <                    + z(i+1,j+1,k) - z(i-1,j+1,k) )
	   jab = xxi * yet * zzt
     <         + xet * yzt * zxi
     <         + xzt * yxi * zet
     <         - xzt * yet * zxi
     <         - xet * yxi * zzt
     <         - xxi * yzt * zet
	   jab = 1.D0 / jab
	   xsx = ( yet * zzt - yzt * zet )
	   esx = ( yzt * zxi - yxi * zzt )
	   zsx = ( yxi * zet - yet * zxi )
	   xsy = ( zet * xzt - zzt * xet )
	   esy = ( zzt * xxi - zxi * xzt )
	   zsy = ( zxi * xet - zet * xxi )
	   xsz = ( xet * yzt - xzt * yet )
	   esz = ( xzt * yxi - xxi * yzt )
	   zsz = ( xxi * yet - xet * yxi )
	   q21(i,j,k) = jab * ( esx * xsx + esy * xsy + esz * xsz )
	   q22(i,j,k) = jab * ( esx * esx + esy * esy + esz * esz )
	   q23(i,j,k) = jab * ( esx * zsx + esy * zsy + esz * zsz )
	   if ( level .eq. 1 ) then
	      etx(i,j,k) = esx
	      ety(i,j,k) = esy
	      etz(i,j,k) = esz
	   endif
	enddo
	enddo
	enddo

C...... K-face
	
	do k = -1, kk+1
	do j =  0, jj+1
	do i =  0, ii+1
	   xzt = ( x(i,j,k+1) - x(i,j,k) )
	   yzt = ( y(i,j,k+1) - y(i,j,k) )
	   zzt = ( z(i,j,k+1) - z(i,j,k) )
	   xxi = 0.25D0 * ( x(i+1,j,k  ) - x(i-1,j,k  ) 
     <                    + x(i+1,j,k+1) - x(i-1,j,k+1) )
	   yxi = 0.25D0 * ( y(i+1,j,k  ) - y(i-1,j,k  ) 
     <                    + y(i+1,j,k+1) - y(i-1,j,k+1) )
	   zxi = 0.25D0 * ( z(i+1,j,k  ) - z(i-1,j,k  ) 
     <                    + z(i+1,j,k+1) - z(i-1,j,k+1) )
	   xet = 0.25D0 * ( x(i,j+1,k  ) - x(i,j-1,k  )
     <                    + x(i,j+1,k+1) - x(i,j-1,k+1) )
	   yet = 0.25D0 * ( y(i,j+1,k  ) - y(i,j-1,k  )
     <                    + y(i,j+1,k+1) - y(i,j-1,k+1) )
	   zet = 0.25D0 * ( z(i,j+1,k  ) - z(i,j-1,k  )
     <                    + z(i,j+1,k+1) - z(i,j-1,k+1) )
	   jab = xxi * yet * zzt
     <         + xet * yzt * zxi
     <         + xzt * yxi * zet
     <         - xzt * yet * zxi
     <         - xet * yxi * zzt
     <         - xxi * yzt * zet
	   jab = 1.D0 / jab
	   xsx = ( yet * zzt - yzt * zet )
	   esx = ( yzt * zxi - yxi * zzt )
	   zsx = ( yxi * zet - yet * zxi )
	   xsy = ( zet * xzt - zzt * xet )
	   esy = ( zzt * xxi - zxi * xzt )
	   zsy = ( zxi * xet - zet * xxi )
	   xsz = ( xet * yzt - xzt * yet )
	   esz = ( xzt * yxi - xxi * yzt )
	   zsz = ( xxi * yet - xet * yxi )
	   q31(i,j,k) = jab * ( zsx * xsx + zsy * xsy + zsz * xsz )
	   q32(i,j,k) = jab * ( zsx * esx + zsy * esy + zsz * esz )
	   q33(i,j,k) = jab * ( zsx * zsx + zsy * zsy + zsz * zsz )
	   if ( level .eq. 1 ) then
	      ztx(i,j,k) = zsx
	      zty(i,j,k) = zsy
	      ztz(i,j,k) = zsz
	   endif
	enddo
	enddo
	enddo
	
C...... Center

	if ( level .eq. 1 ) then
	   do k = 0, kk+1
	   do j = 0, jj+1
	   do i = 0, ii+1
	      xxi = 0.5D0 * ( x(i+1,j,k) - x(i-1,j,k) )
	      yxi = 0.5D0 * ( y(i+1,j,k) - y(i-1,j,k) )
	      zxi = 0.5D0 * ( z(i+1,j,k) - z(i-1,j,k) )
	      xet = 0.5D0 * ( x(i,j+1,k) - x(i,j-1,k) )
	      yet = 0.5D0 * ( y(i,j+1,k) - y(i,j-1,k) )
	      zet = 0.5D0 * ( z(i,j+1,k) - z(i,j-1,k) )
	      xzt = 0.5D0 * ( x(i,j,k+1) - x(i,j,k-1) )
	      yzt = 0.5D0 * ( y(i,j,k+1) - y(i,j,k-1) )
	      zzt = 0.5D0 * ( z(i,j,k+1) - z(i,j,k-1) )
	      jab = xxi * yet * zzt
     <            + xet * yzt * zxi
     <            + xzt * yxi * zet
     <            - xzt * yet * zxi
     <            - xet * yxi * zzt
     <            - xxi * yzt * zet
	      jab = 1.D0 / jab
	      jac(i,j,k) = jab
	      xsx = ( yet * zzt - yzt * zet )
	      esx = ( yzt * zxi - yxi * zzt )
	      zsx = ( yxi * zet - yet * zxi )
	      xsy = ( zet * xzt - zzt * xet )
	      esy = ( zzt * xxi - zxi * xzt )
	      zsy = ( zxi * xet - zet * xxi )
	      xsz = ( xet * yzt - xzt * yet )
	      esz = ( xzt * yxi - xxi * yzt )
	      zsz = ( xxi * yet - xet * yxi )
	     
	   enddo
	   enddo
	   enddo
	   do k = -1, kk+2
	   do j = -1, jj+2
	   do i = -1, ii+2
	      phi_init(i,j,k) = 0.D0
	   enddo
	   enddo
	   enddo
	endif

	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   q12(i,j,k) = 0.25D0 * q12(i,j,k)
	   q13(i,j,k) = 0.25D0 * q13(i,j,k)
	   q23(i,j,k) = 0.25D0 * q23(i,j,k)
	   q21(i,j,k) = 0.25D0 * q21(i,j,k)
	   q31(i,j,k) = 0.25D0 * q31(i,j,k)
	   q32(i,j,k) = 0.25D0 * q32(i,j,k)
	enddo
	enddo
	enddo

	do k = 0, kk+2
	do j = 0, jj+2
	do i = 0, ii+2
	   qcc(i,j,k) = q11(i,j,k) + q11(i-1,j,  k  )
     <                + q22(i,j,k) + q22(i,  j-1,k  )
     <                + q33(i,j,k) + q33(i,  j,  k-1)
	enddo 
	enddo
	enddo

	return
	end
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	subroutine pres_corr

	include "size.inc"
	include "mpif.h"
	include "mpi.inc"
	include "metric.inc"
	include "mygrid.inc"
	include "para.inc"
	include "sedi.inc"
	include "ns.inc"

	integer i,j,k,m
	double precision tmp

	do m = 1, 3
	do k = 0, nnk
	do j = 0, nnj
	do i = 0, nni
	   pcorr(i,j,k,m) = 1.D0/(1.D0*(1.D0-sedf(i,j,k,m))
     <                     +spwght*sedf(i,j,k,m))
	enddo
	enddo
	enddo
	enddo
	
	return
	end



ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	subroutine qf2c(iif, jjf, kkf, iic, jjc, kkc, jaq, jas,
     <          q11, q12, q13, q21, q22, q23, q31, q32, q33, qcc,
     <          s11, s12, s13, s21, s22, s23, s31, s32, s33, scc)

	implicit none	

	integer iif, jjf, kkf, iic, jjc, kkc

	double precision jaq(-1:iif+2,-1:jjf+2,-1:kkf+2),
     &                   q11(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q12(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q13(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q21(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q22(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q23(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q31(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q32(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   q33(-1:iif+2,-1:jjf+2,-1:kkf+2), 
     &                   qcc(-1:iif+2,-1:jjf+2,-1:kkf+2)
     
	double precision jas(-1:iic+2,-1:jjc+2,-1:kkc+2),
     &                   s11(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s12(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s13(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s21(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s22(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s23(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s31(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s32(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   s33(-1:iic+2,-1:jjc+2,-1:kkc+2), 
     &                   scc(-1:iic+2,-1:jjc+2,-1:kkc+2)

	integer i, j, k, i0, j0, k0

	do k = -1, kkc+2
	do j = -1, jjc+2
	do i = -1, iic+2
	   jas(i,j,k) = 0.D0
	   s11(i,j,k) = 0.D0
	   s12(i,j,k) = 0.D0
	   s13(i,j,k) = 0.D0
	   s21(i,j,k) = 0.D0
	   s22(i,j,k) = 0.D0
	   s23(i,j,k) = 0.D0
	   s31(i,j,k) = 0.D0
	   s32(i,j,k) = 0.D0
	   s33(i,j,k) = 0.D0
	enddo
	enddo
	enddo

	do k = 1, kkc
	   k0 = 2 * k - 1
	do j = 1, jjc
	   j0 = 2 * j - 1
	do i = 0, iic
	   i0 = 2 * i - 1
	   jas(i,j,k) = 1.D0 / jaq(i0,  j0,  k0  )
     <	              + 1.D0 / jaq(i0+1,j0,  k0  )
     <	              + 1.D0 / jaq(i0  ,j0+1,k0  )
     <	              + 1.D0 / jaq(i0+1,j0+1,k0  )
     <	              + 1.D0 / jaq(i0,  j0,  k0+1)
     <	              + 1.D0 / jaq(i0+1,j0,  k0+1)
     <	              + 1.D0 / jaq(i0,  j0+1,k0+1)
     <	              + 1.D0 / jaq(i0+1,j0+1,k0+1)
	   jas(i,j,k) = 1.D0 / jas(i,j,k)
	enddo
	enddo
	enddo

	do k = 1, kkc
	   k0 = 2 * k - 1
	do j = 1, jjc
	   j0 = 2 * j - 1
	do i = 0, iic
	   i0 = 2 * i
	      s11(i,j,k) = 0.5D0 * ( q11(i0,j0,  k0  ) 
     <	                           + q11(i0,j0+1,k0  )
     <	                           + q11(i0,j0  ,k0+1)
     <	                           + q11(i0,j0+1,k0+1) )
	      s12(i,j,k) = 0.5D0 * ( q12(i0,j0,  k0  ) 
     <	                           + q12(i0,j0+1,k0  )
     <	                           + q12(i0,j0  ,k0+1)
     <	                           + q12(i0,j0+1,k0+1) )
	      s13(i,j,k) = 0.5D0 * ( q13(i0,j0,  k0  ) 
     <	                           + q13(i0,j0+1,k0  )
     <	                           + q13(i0,j0  ,k0+1)
     <	                           + q13(i0,j0+1,k0+1) )
	enddo
	enddo
	enddo

	do k = 1, kkc
	   k0 = 2 * k - 1
	do j = 0, jjc
	   j0 = 2 * j
	do i = 1, iic
	   i0 = 2 * i - 1
	      s21(i,j,k) = 0.5D0 * ( q21(i0,  j0,k0  ) 
     <	                           + q21(i0+1,j0,k0  )
     <	                           + q21(i0,  j0,k0+1)
     <	                           + q21(i0+1,j0,k0+1) )
	      s22(i,j,k) = 0.5D0 * ( q22(i0,  j0,k0  ) 
     <	                           + q22(i0+1,j0,k0  )
     <	                           + q22(i0,  j0,k0+1)
     <	                           + q22(i0+1,j0,k0+1) )
	      s23(i,j,k) = 0.5D0 * ( q23(i0,  j0,k0  ) 
     <	                           + q23(i0+1,j0,k0  )
     <	                           + q23(i0,  j0,k0+1)
     <	                           + q23(i0+1,j0,k0+1) )
	enddo
	enddo
	enddo

	do k = 0, kkc
	   k0 = 2 * k
	do j = 1, jjc
	   j0 = 2 * j - 1
	do i = 1, iic
	   i0 = 2 * i - 1
	      s31(i,j,k) = 0.5D0 * ( q31(i0,  j0,  k0) 
     <	                           + q31(i0+1,j0,  k0)
     <	                           + q31(i0,  j0+1,k0)
     <	                           + q31(i0+1,j0+1,k0) )
	      s32(i,j,k) = 0.5D0 * ( q32(i0,  j0,  k0) 
     <	                           + q32(i0+1,j0,  k0)
     <	                           + q32(i0,  j0+1,k0)
     <	                           + q32(i0+1,j0+1,k0) )
	      s33(i,j,k) = 0.5D0 * ( q33(i0,  j0,  k0) 
     <	                           + q33(i0+1,j0,  k0)
     <	                           + q33(i0,  j0+1,k0)
     <	                           + q33(i0+1,j0+1,k0) )
	enddo
	enddo
	enddo

	do k = 1, kkc
	do j = 1, jjc
	do i = 1, iic
	   scc(i,j,k) = s11(i,j,k) + s11(i-1,j,  k  )
     <	              + s22(i,j,k) + s22(i,  j-1,k  )
     <	              + s33(i,j,k) + s33(i,  j,  k-1)
	enddo 
	enddo
	enddo

	return
	end
     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine mapx(ii, jj, kk, x, xi)

	include "size.inc"
	include "mpif.h"
	include "mygrid.inc"
	include "mpi.inc"

	integer ii, jj, kk

	double precision x(-1:ii+2,-1:jj+2,-1:kk+2),
     &                   xi(-1:ii+2,-1:jj+2,-1:kk+2)

	integer i, j, k
	
	if ( stretchx .eq. 1 ) then

	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   x(i,j,k) = dm * xi(i,j,k) 
     <              + dlog( dcosh( am * ( xi(i,j,k) - bm ) )
     <                    / dcosh( am *               bm   ) ) 
     <              / am
     <              - dlog( dcosh( am * ( xi(i,j,k) - 1.D0+bm ) )
     <                    / dcosh( am * (             1.D0-bm ) ) )
     <              / am
	   x(i,j,k) = cm * x(i,j,k)
	enddo
	enddo
	enddo

CBCBCBCBCBC
	IF ( N_WEST .EQ. MPI_PROC_NULL ) THEN
	DO K = -1, KK+2
	DO J = -1, JJ+2
	   X( 0,J,K) = - X(1,J,K)
	   X(-1,J,K) = - X(2,J,K)
	ENDDO
	ENDDO
	ENDIF
	IF ( N_EAST .EQ. MPI_PROC_NULL ) THEN
	DO K = -1, KK+2
	DO J = -1, JJ+2
	   X(II+1,J,K) = 2.D0 - X(II,  J,K)
	   X(II+2,J,K) = 2.D0 - X(II-1,J,K)
	ENDDO
	ENDDO
	ENDIF
CBCBCBCBCBC

	else

	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   x(i,j,k) = xi(i,j,k)
	enddo
	enddo
	enddo

	endif

	return
	end

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine mapy(ii, jj, kk, x, xi)

	include "size.inc"
	include "mpif.h"
	include "mygrid.inc"
	include "mpi.inc"
        include "metric.inc"

	integer nd, ii, jj, kk

	double precision x(-1:ii+2,-1:jj+2,-1:kk+2),
     &                   xi(-1:ii+2,-1:jj+2,-1:kk+2)

	integer i, j, k
        double precision dp, ap
	
        ap = 9.D0

	

	if ( stretchy .eq. 1 ) then

        dm = 0.D0
	am = 4.8D0
	bm = 0.48D0
        
	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
c           dp = depth(i,k)
c	   cm = dm
c     *	   + dlog( dcosh(am*(dp-bm)) / dcosh(am*bm) )/am
c     *     - dlog( dcosh(am*bm) / dcosh(am*(dp-bm)) )/am
c  	   cm = dp/cm


c	   x(i,j,k) = dm * (xi(i,j,k)+dp) 
c     <              + dlog( dcosh( am * ( xi(i,j,k)+dp - bm ) )
c     <                    / dcosh( am *                  bm ) ) 
c     <              / am
c     <              - dlog( dcosh( am * ( xi(i,j,k)+dp - dp +bm ) )
c     <                    / dcosh( am * (                dp -bm ) ) )
c     <              / am
c	   x(i,j,k) = cm * x(i,j,k) - dp
           dp = depth(i, k)
c           dp = -xi(i, -1, k)
c           dt = xi(i, nnj+2, k)
           x(i, j, k) = (dexp(ap*xi(i, j, k))-dexp(-ap*dp))
     <                  / (dexp(ap*0.D0)-dexp(-ap*dp))
     <                  * dp - dp
      
	enddo
	enddo
	enddo

CBCBCBCBCBC
c	IF ( N_SUTH .EQ. MPI_PROC_NULL ) THEN
c	DO K = -1, KK+2
c	DO I = -1, II+2
c	   X(I, 0,K) = - X(I,1,K)
c	   X(I,-1,K) = - X(I,2,K)
c	ENDDO
c	ENDDO
c	ENDIF
c	IF ( N_NRTH .EQ. MPI_PROC_NULL ) THEN
c	DO K = -1, KK+2
c	DO I = -1, II+2
c	   X(I,JJ+1,K) = 2.D0 - X(I,JJ,  K)
c	   X(I,JJ+2,K) = 2.D0 - X(I,JJ-1,K)
c	ENDDO
c	ENDDO
c	ENDIF
CBCBCBCBCBC

	else

	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   x(i,j,k) = xi(i,j,k)
	enddo
	enddo
	enddo

	endif

	return
	end

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine mapz(ii, jj, kk, x, xi)

	include "size.inc"
	include "mpif.h"
	include "mygrid.inc"
	include "mpi.inc"

	integer nd, ii, jj, kk

	double precision x(-1:ii+2,-1:jj+2,-1:kk+2),
     &                   xi(-1:ii+2,-1:jj+2,-1:kk+2)

	integer i, j, k
	
	if ( stretchz .eq. 1 ) then

	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   x(i,j,k) = dm * xi(i,j,k) 
     <              + dlog( dcosh( am * ( xi(i,j,k) - bm ) )
     <                    / dcosh( am *               bm   ) ) 
     <              / am
     <              - dlog( dcosh( am * ( xi(i,j,k) - 1.D0+bm ) )
     <                    / dcosh( am * (             1.D0-bm ) ) )
     <              / am
	   x(i,j,k) = cm * x(i,j,k)
	enddo
	enddo
	enddo
        
CBCBCBCBCBC
	IF ( N_BACK .EQ. MPI_PROC_NULL ) THEN
	DO J = -1, JJ+2
	DO I = -1, II+2
	   X(I,J, 0) = - X(I,J,1)
	   X(I,J,-1) = - X(I,J,2)
	ENDDO
	ENDDO
	ENDIF
	IF ( N_FRNT .EQ. MPI_PROC_NULL ) THEN
	DO J = -1, JJ+2
	DO I = -1, II+2
	   X(I,J,KK+1) = 2.D0 - X(I,J,KK  )
	   X(I,J,KK+2) = 2.D0 - X(I,J,KK-1)
	ENDDO
	ENDDO
	ENDIF
CBCBCBCBCBC
        
	else
        
	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
 
           x(i,j,k) = xi(i,j,k)
           
	enddo
	enddo
	enddo
        
	endif

	return
	end

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine coordxz(ii, jj, kk, x, z, xi, zt)

	include "size.inc"
	include "mygrid.inc"

	integer ii, jj, kk
         
	double precision x(-1:ii+2,-1:jj+2,-1:kk+2),
     &                   y(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   z(-1:ii+2,-1:jj+2,-1:kk+2)

	double precision xi(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   et(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   zt(-1:ii+2,-1:jj+2,-1:kk+2)

	integer i, j, k
	call mapx(ii, jj, kk, x, xi)	
        call mapz(ii, jj, kk, z, zt)
        
	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   x(i,j,k) = bx * x(i,j,k)
	   z(i,j,k) = bz * z(i,j,k) 
	enddo
	enddo
	enddo

	return
	end
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

	subroutine coordy(ii, jj, kk, x, y, z, xi, et, zt)

	include "size.inc"
	include "mygrid.inc"

	integer ii, jj, kk

	double precision x(-1:ii+2,-1:jj+2,-1:kk+2),
     &                   y(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   z(-1:ii+2,-1:jj+2,-1:kk+2)

	double precision xi(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   et(-1:ii+2,-1:jj+2,-1:kk+2), 
     &                   zt(-1:ii+2,-1:jj+2,-1:kk+2)
        
	
        integer i, j, k
	
	call mapy(ii, jj, kk, y, et)
	
	do k = -1, kk+2
	do j = -1, jj+2
	do i = -1, ii+2
	   y(i,j,k) = by * y(i,j,k) 
	enddo
	enddo
	enddo

	return
	end

ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	subroutine returnDepth(ii, jj, kk, x, z, depth, depth0)
	  
	include "para.inc"

        double precision A, fx, fz, x0, z0
        double precision r, rr, ht, temp
        
        integer ii, jj, kk, i, j, k

	double precision x(-1:ii+2, -1:jj+2, -1:kk+2),
     &                   z(-1:ii+2, -1:jj+2, -1:kk+2),
     &               depth(-1:ii+2, -1:kk+2),
     &              depth0(-1:ii+2, -1:kk+2),
     &                  hh(-1:ii+2, -1:kk+2)	
c     &                 tz1(-1:ii+2, -1:kk+2),
c     &                 tz2(-1:ii+2, -1:kk+2)

        A = 0.02D0

	do k = -1, kk+2
        do i = -1, ii+2
	   
           depth0(i,k) = 0.15D0

	enddo
        enddo
	

	x0 = 0.3
	z0 = 0.18
	rr = 0.12
	ht = 0.05

	do k = -1, kk+2
        do i = -1, ii+2
c	   depth(i, k) = depth0(i,k) - 0.07*dexp(-((x(i,1,k)-0.5)**2.0)/0.006)

c	   if (x(i,1,k) .le. 0.6 .and. x(i,1,k) .ge. 0.4) then
c	      depth(i,k) = 0.15
c	   else
c	      depth(i,k) = 0.2
c          endif

c	   r = dsqrt((x(i,1,k)-x0)**2.D0+(z(i,1,k)-z0)**2.D0)
c	   if (r/rr .le. 1) then
c	      temp = ht*(dcos(pi*r/(2.D0*rr)))**2.D0
c	   else
c	      temp = 0.D0
c	   endif
c	   depth(i, k) = depth0(i,k) - temp
	   depth(i, k) = depth0(i,k) +0.00*dcos(2*pi/0.25*(x(i,1,k)))
c           depth(i, k) = 0.2-0.02*time*dexp(-(x(i,1,k)-0.25)**2.D0/
c     <                   0.005)
c	   depth(i, k) = 0.05D0 - 0.0D0*1.D0/(0.15D0*sqrt(2*3.1415916))
c     <                   *dexp(-(((x(i, 1, k)-0.5)**2)
c     <                         /(2*0.15D0**2)))
c	   depth(i, k) = 0.3D0 + 0.0D0*dcos(x(i, 1, k)/1.D0
c     <                 *2.D0*3.1415916D0)
c           depth(i, k) = 0.3D0+amp*dcos(x(i, 1, k)/0.5D0
c     <                   *2.D0*3.141516D0)
c           fx = -amp*2.D0/0.25D0*3.141516D0
c     <          *dsin(x(i, 1, k)/0.25D0*2.D0*3.141516D0)
c	   fx = 1.D0
c	   fz = 0.D0
c	   tx1(i,k) = 1.D0/dsqrt(1 + fx**2)
c	   tx2(i,k) = fx*tx1(i,k)
c	   tz2(i,k) = 1.D0/dsqrt(1 + fz**2)
c           tz1(i,k) = fz*tz2(i,k)
c          write(*,*) 'tx = ', tz1(i,k)
        enddo
	enddo
	
	return 
	end
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 	subroutine smoothDepth

	include "size.inc"
	include "para.inc"
	include "ns.inc"
	include "metric.inc"
	          
        integer i, j, k, kap

	double precision temp(-1:nni+2, -1:nnk+2)


	do k = -1, nnk+2
        do i = -1, nni+2
	   temp(i, k) = depth(i,k)
        enddo
	enddo

	do k = 0, nnk+1
        do i = 0, nni+1
	   depth(i,k) = 1.D0/9.D0*(
     <         temp(i-1,k-1) +   temp(i-1,k) +   temp(i-1,k+1) +
     <         temp(i  ,k-1) +   temp(i  ,k) +   temp(i  ,k+1) +
     <         temp(i+1,k-1) +   temp(i+1,k) +   temp(i+1,k+1))
	   
        enddo
	enddo
	
	do i = 0, nni+1
	   depth(i, nnk+2) = depth(i, nnk+1)
	   depth(i, -1) = depth(i, 0)
        enddo

	do k = 0, nnk+1
	   depth(nni+2, k) = depth(nni+1, k)
	   depth(-1, k) = depth(0, k)
        enddo

	depth(nni+2, -1) = depth(nni+1, 0)
	depth(nni+2, nnk+2) = depth(nni+2, nnk+1)
	depth(-1, -1) = depth(0, 0)
	depth(-1, nnk+2) = depth(0, nnk+1)
	
	    
	return 
	end
