...
c	      end if
              ...
              do j = -1, nnj+2
              do i = -1, nni+2
c                zeta = -0.1D0*exp(-(xp(i,j,k,1)/0.7D0)**2) 
c     <                  + 0.001D0*ran0(idum)
                 phi(i,j,k) = -0.03D0*(xp(i,j,k,2)-1) + 0.985D0
c                phi(i,j,k) = 1.D0-0.5D0*0.03D0
c    <            *tanh(2D0*(xp(i,j,k,2)+0.45D0)/0.02D0*2.64665D0)
              enddo
              enddo
              enddo
              call phi_bc
