...
rho_pert_pcui = zeros(size(x)); %-0.5*0.03*tanh(2*(y - 0.1*cos(4*pi*x) - D/2)/0.2*atanh(0.99));

rho_full_pcui = rho_init_pcui+rho_pert_pcui;

omega = 2*pi/params.Twave;
delta = sqrt(omega/(2*params.molecular_viscosity));
u_pcui = params.waveMag*cos(omega*(0)*params.dt*params.nsave)-params.waveMag*exp(-y/delta).*cos(omega*(0)*params.dt*params.nsave-y/delta);

v_pcui = zeros(size(rho_init_pcui)); w_pcui = zeros(size(rho_init_pcui));
uvw_pcui(:,:,:,1) = u_pcui;
uvw_pcui(:,:,:,2) = v_pcui;
uvw_pcui(:,:,:,3) = w_pcui;
